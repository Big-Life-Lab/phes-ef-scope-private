...
need <- c("term_name","obo_id","term_label","see_also","scr_def",
          "term_description","term_background","eval_methods","scr_exact_synonym","scr_broad_synonym")
for (c in need) if (!c %in% names(d)) d[[c]] <- NA_character_

d <- d %>%
  select(all_of(need)) %>%
  filter(!is.na(term_name), term_name != "", term_name != "N/A") %>%
...
# Render markdown in description, background, and eval_methods fields
d <- d %>%
  rowwise() %>%
  mutate(
    ...
    eval_methods_html = render_markdown(eval_methods, term_name, "eval_methods")
  ) %>%
  ungroup()
```
...
purrr::pwalk(
  ...
  function(term_name, obo_id, term_label, see_also, scr_def,
           term_description, term_background, eval_methods, scr_exact_synonym, scr_broad_synonym,
           header, anchor, see_also_render, term_description_html, term_background_html, eval_methods_html) {
...
**Synonyms:** **Exact:** {scr_exact_synonym} • **Broad:** {scr_broad_synonym}

<hr class=\"header-separator\">

**Definition:** {scr_def}

**Description:** {term_description_html}

**Background:** {term_background_html}

**Measurement:** {eval_methods_html}
